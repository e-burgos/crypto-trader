...
> Última actualización: cycle-02 | Fecha: 2026-09-03
...
- **Estado:** proyecto pre-existente adoptado por el arnés SDD.
  ...
  - **spec-e-burgos-005 cycle-02** — entradas descansando en el exchange: `OrderExecutorPort` pasa de 9 a 12 métodos (`placeEntryOrder`/`getEntryOrderStatus`/`cancelEntryOrder`), nueva función pura `resolveEntryLevels` (soporte/resistencia → niveles de entrada) y `ENTRY_CANCEL` como `kind` `REDUCING` en `action-caps.ts`. Ver `constitution.md` §3.
