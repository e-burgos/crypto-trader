...
import { AlertTriangle, Activity, TrendingUp, Users, BarChart3, Clock, Loader2 } from 'lucide-react';
import { Button } from '../../components/ui/button';
import { InfoTooltip } from '../../components/ui/info-tooltip';
import { cn } from '../../lib/utils';
import { useGSAP } from '@gsap/react';
import gsap from 'gsap';
import { useAdminStats, useAuditLog, useAdminAgentsStatus, useKillSwitch } from '../../hooks/use-admin';
  ...
  return (
    <div ref={containerRef} className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        ...
        <StatCard
          label="P&L Today"
          value={stats ? `$${stats.profitToday.toFixed(2)}` : '–'}
          icon={TrendingUp}
          color={stats && stats.profitToday >= 0 ? 'text-emerald-500' : 'text-red-500'}
          tooltip="Net profit/loss generated by all agents today"
        />
      </div>
            ...
            {agentStatuses.map((s, i) => (
              <div
                ...
                className={cn(
                  ...
                  s.isRunning ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-border',
                )}
              >
                <span className="font-medium">{s.asset}/{s.pair} ({s.mode})</span>
                <span className={cn('text-xs font-semibold', s.isRunning ? 'text-emerald-500' : 'text-muted-foreground')}>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <h3 className="mb-4 font-semibold flex items-center gap-2">
          ...
          Audit Log
          <span className="text-xs text-muted-foreground font-normal">(last 50 actions)</span>
        </h3>
        <div className="max-h-64 overflow-y-auto space-y-1">
          {auditLog.length === 0 ? (
            ...
            auditLog.slice(0, 50).map((entry) => (
              <div key={entry.id} className="flex items-start justify-between gap-3 rounded-md px-2 py-1.5 text-xs hover:bg-muted/30">
