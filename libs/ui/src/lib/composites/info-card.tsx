...
export interface InfoCardColumn {
  ...
  /** Accent color class for the vertical bar (e.g. 'bg-primary') */
  accent?: string;
  /** Accent text color class for label and icon (e.g. 'text-primary') */
  accentText?: string;
  ...
  return (
    ...
    >
      ...
      {columns && columns.length > 0 && (
        <div
          className={cn(
            'grid sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-border/30',
              ...
              <div className="flex items-center gap-2">
                <div
                  className={cn(
                    ...
                    col.accent ?? 'bg-primary',
                  )}
                />
                {col.icon && (
                  <span
                    className={cn('h-3 w-3', col.accentText ?? 'text-primary')}
                  >
                    {col.icon}
                  </span>
                )}
            ...
            footerTags.length > 0 && (
              <div className="px-5 py-3 border-t border-border/40 bg-muted/10 flex flex-wrap gap-2 items-center">
                ...
                {footerTags.map((tag, i) => {
                  ...
                  return tag.tooltip ? (
                    <Tooltip key={i} content={tag.tooltip} side="top">
                      {tagEl}
                    </Tooltip>
                  ) : (
                    tagEl
