...
const PAIRS = ['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'SOLUSDT', 'ADAUSDT', 'DOTUSDT', 'AVAXUSDT'];
...
export function ConfigPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      toast.error((err as { message?: string })?.message || 'Failed to save config');
    } finally {
      setIsLoading(false);
    }
  }
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">Set up your AI trading agent parameters</p>
      </div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                value={form.pair}
                onChange={(e) => update({ pair: e.target.value, asset: e.target.value.replace('USDT', '') })}
                ...
                {(['PAPER', 'LIVE'] as TradingMode[]).map((m) => (
                  <button
                    ...
                    className={cn(
                      ...
                      form.mode === m ? 'border-primary bg-primary/10 text-primary' : 'border-border hover:border-primary/40',
                    )}
                  >
                    {m}
                  </button>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {[
              ...
              { key: 'agentIntervalMinutes', label: 'Agent Interval', suffix: 'min' },
              ...
              <div key={key}>
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(e) => update({ [key]: e.target.value } as Partial<ConfigForm>)}
