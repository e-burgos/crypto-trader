...
- **Estado:** proyecto pre-existente adoptado por el arnés SDD.
  ...
  - **spec-e-burgos-010 cycle-01, cerrado sin aprobar (DEC-001).** `resolveUserDataStreamHealth()` nuevo en `src/lib/reactive/`: función pura con tests verdes, hermana de `resolveStreamHealth` pero con contrato propio. Hoy sin llamador real: su único consumidor (`UserDataStreamService` de `apps/api`) es inerte porque Binance retiró el endpoint del `listenKey`.
  - **spec-e-burgos-010 cycle-02** — `resolveUserDataStreamHealth()` siguió al cambio de transporte: `KEEPALIVE_STALE` pasó a `SESSION_AUTH_STALE` (umbral `sessionAuthMaxAgeMs`), con tests nuevos que protegen RN-03. Ver `constitution.md` §3.
...
- `resolveUserDataStreamHealth()` sigue sin llamador vivo en producción: su único consumidor (`UserDataStreamService`, transporte WebSocket API desde cycle-02) sigue detrás de `USER_DATA_STREAM_FILLS_ENABLED`, apagado hasta la corrida autenticada contra TESTNET.
