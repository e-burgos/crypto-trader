...
export function DocsPageFeedback({ className }: DocsPageFeedbackProps) {
  const [feedback, setFeedback] = useState<'yes' | 'no' | null>(null);

  return (
    <div
      className={cn(
        'flex items-center justify-center gap-4 rounded-lg border border-border/60 bg-card px-6 py-5',
        className,
      )}
    >
      {feedback ? (
